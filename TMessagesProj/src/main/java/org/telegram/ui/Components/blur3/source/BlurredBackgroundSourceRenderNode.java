...
public class BlurredBackgroundSourceRenderNode implements BlurredBackgroundSource {
    ...
    private RuntimeShader progressiveShaderVertical;
    ...
    private static final String PROGRESSIVE_BLUR_SHADER =
        ...
        "        sampleCoord.x = clamp(sampleCoord.x + float(i) * stride, 0.0, textureWidth);\n" +
    ...
    private static final String PROGRESSIVE_BLUR_SHADER_VERTICAL =
        "uniform shader inputTexture;\n" +
        "uniform float maxRadius;\n" +
        "uniform float textureWidth;\n" +
        "uniform float textureHeight;\n" +
        ...
        "        float weight = exp(-float(i * i) * invTwoSigma2);\n" +
        "        float2 sampleCoord = coord;\n" +
        "        sampleCoord.y = clamp(sampleCoord.y + float(i) * stride, 0.0, textureHeight);\n" +
        "        result += inputTexture.eval(sampleCoord) * half4(weight);\n" +
        "        totalWeight += weight;\n" +
        "    }\n" +
        "    return result / half4(totalWeight);\n" +
    ...
    public void setProgressiveBlur(float maxRadius, int sourceWidth, int sourceHeight, float fadeZoneTopFraction, float fadeZoneBottomFraction, int samples) {
        ...
        if (progressiveShader == null) {
            ...
            progressiveShaderVertical = new RuntimeShader(PROGRESSIVE_BLUR_SHADER_VERTICAL);
        }
        ...
        progressiveShaderVertical.setFloatUniform("maxRadius", maxRadius);
        progressiveShaderVertical.setFloatUniform("textureWidth", (float) sourceWidth);
        progressiveShaderVertical.setFloatUniform("textureHeight", (float) sourceHeight);
        progressiveShaderVertical.setFloatUniform("fadeZoneTopFraction", topFraction);
        progressiveShaderVertical.setFloatUniform("fadeZoneBottomFraction", bottomFraction);
        progressiveShaderVertical.setIntUniform("samples", sampleCount);
        renderNode.setRenderEffect(RenderEffect.createChainEffect(
            RenderEffect.createRuntimeShaderEffect(progressiveShader, "inputTexture"),
            RenderEffect.createRuntimeShaderEffect(progressiveShaderVertical, "inputTexture")
        ));
        lastBlurRadius = -1f;
    }
