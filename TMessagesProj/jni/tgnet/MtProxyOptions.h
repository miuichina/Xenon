 ...
 *
 * Subset of ZaStoGram MTProxy masking options, ported into Xenon behind the
 * "Bypass blocking" toggle. Only the TLS-profile and ClientHello-fragmentation
 * layers are wired up; the rest of the struct is preserved verbatim so the
 * shape stays compatible with upstream ZaStoGram if more layers are ported.
 */
...
static inline int32_t normalizeMtProxyTlsProfileOption(int32_t value) {
    ...
    if (value >= MT_PROXY_TLS_PROFILE_FIREFOX && value <= MT_PROXY_TLS_PROFILE_ANDROID_OKHTTP) {
        return value;
    }
